...
module.exports = {
    ...
    async execute(interaction) {
        ...
        const fromInput = (interaction.options.getString('from') || 'UTC').toUpperCase();
        ...
        if (!moment.tz.zone(sourceZone) && !sourceZone.startsWith('UTC')) {
            return interaction.reply({ content: `❌ Invalid Source: \`${fromInput}\``, ephemeral: true });
        }
        if (!moment.tz.zone(targetZone) && !targetZone.startsWith('UTC')) {
            return interaction.reply({ content: `❌ Invalid Target: \`${toInput}\``, ephemeral: true });
        }
        ...
        if (!baseTime.isValid()) {
            return interaction.reply({ content: `❌ Invalid time format. Try \`0:00\`.`, ephemeral: true });
        }
        ...
        await interaction.reply(`\`${baseTime.format('HH:mm')} ${fromInput}\` \u2192 \`${convertedTime.format('HH:mm')}\`${dayDiffString} ${toInput} Time`);
    },
};